...
# What kind of chart do you get? Stacked bar graph
ggplot(data = diamonds.sample) +
...
ggplot(data = diamonds.sample) +
  geom_histogram(mapping = aes(x = clarity), stat = "count")

# (For a more traditional "bell-curve", make a histogram of diamond `depths`)
ggplot(data = diamonds.sample) +
  geom_histogram(mapping = aes(x = depth), stat = "count")
...
diamond.summary <- diamonds.sample %>%
  group_by(clarity) %>%
  summarize(mean = mean(price), standard_error = sd(price) / sqrt(n()))

# Then draw the plot. The error bars should stretch from the mean-error to the mean+error.

ggplot(data = diamond.summary) +
  geom_col(mapping = aes(x = clarity, y = mean)) +
  geom_errorbar(mapping = aes(x = clarity, ymin = mean - standard_error, ymax = mean + standard_error), width = 0.4)
